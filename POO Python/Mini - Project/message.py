...
class IntervalTimerApp:
    def __init__(self, root):
        ...
        self.background_photo = ImageTk.PhotoImage(
            self.background_image.resize((1000, 800))
        )
    ...
    def create_widgets(self):
        ...
        ttk.Label(
            input_frame, text="Nombre de séries:", style="InputLabel.TLabel"
        ).grid(row=0, column=0, sticky="w")
        ttk.Entry(input_frame, textvariable=self.num_sets_var).grid(row=0, column=1)

        ttk.Label(
            input_frame, text="Nombre d'exercices par série:", style="InputLabel.TLabel"
        ).grid(row=1, column=0, sticky="w")
        ttk.Entry(input_frame, textvariable=self.exercises_per_set_var).grid(
            row=1, column=1
        )

        ttk.Label(
            input_frame,
            text="Durée de travail par exercice (en secondes):",
        ...
        ttk.Label(
            input_frame,
            text="Temps de repos entre les exercices (en secondes):",
            style="InputLabel.TLabel",
        ...
        ttk.Label(
            input_frame,
            text="Temps de repos entre les séries (en secondes):",
            style="InputLabel.TLabel",
        ).grid(row=4, column=0, sticky="w")
        ttk.Entry(input_frame, textvariable=self.rest_between_sets_var).grid(
            row=4, column=1
        )
        ...
        self.start_button = ttk.Button(
            button_frame, text="Start", command=self.start_timer, style="Green.TButton"
        )
        self.start_button.grid(row=0, column=0, padx=5)

        self.pause_button = ttk.Button(
            button_frame, text="Pause", command=self.pause_timer, style="Orange.TButton"
        )
        self.pause_button.grid(row=0, column=1, padx=5)

        self.stop_button = ttk.Button(
            button_frame, text="Stop", command=self.stop_timer, style="Red.TButton"
        )
    ...
    def start_timer(self):
        ...
        self.interval_timer = IntervalTimer(
            num_sets, exercises_per_set, work_time, rest_time, rest_between_sets
        )
...
class IntervalTimer:
    def __init__(
        self, num_sets, exercises_per_set, work_time, rest_time, rest_between_sets
    ):
...
def main():
    ...
    root.style.configure(
        "Red.TButton", foreground="black", background="red", font=("Arial", 12, "bold")
    )
