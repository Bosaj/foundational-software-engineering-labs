...
def moyenne_eleves_map_lambda(notes_eleves):
    return dict(map(lambda x: (x[0], sum(x[1]) / len(x[1])), notes_eleves.items()))

# Méthode 4 : Utilisation de la fonction zip() avec une fonction lambda
def moyenne_eleves_zip_lambda(notes_eleves):
    return dict(zip(notes_eleves.keys(), map(lambda x: sum(x) / len(x), notes_eleves.values())))
...
print("Méthode 2 (compréhension de dictionnaire) :", moyenne_eleves_comprehension_dict(notes_eleves))
